//scripts Jibo missions 
/* <script src="https://code.jquery.com/jquery-3.2.1.js"
    integrity="sha256-DZAnKJ/6XZ9si04Hgrsxu/8s717jcIzLy3oi35EouyE="
    crossorigin="anonymous">
</script>; */
{/* <script src="./missions/secret/certificate.js"></script> */}
// require('https://code.jquery.com/jquery-3.2.1.js');
// require('./missions/secret/certificate.js');
// require('./missions/secret/renv-client.js');
// require('./missions/secret/media-images.js');
// // const jibo = require('remote-jibo-node');
// window.robot = null;

// function onWrapped(ws) {
...
// function doConnection(runtime) {
//   client.ws.onmessage = (msg => {
//       console.log("MSG: " + msg.data);
//       msg = JSON.parse(msg.data);
//       if (msg.type === 'handshake'){
//           window.robot = jibo.RobotClient.fromWebSocket(msg.name, onWrapped(client.ws));
//           console.log("Connected: " + robot);

//           //heyjibo event hat launch
//           window.robot.on(jibo.RobotEvents.HEY_JIBO, function() {
//               console.log("hey jibo event");
//               let asrResultFunction = function (result) {
//                   window.robot.removeListener(jibo.RobotEvents.ASR_RESULT, asrResultFunction);
//                   asrResult = result.input;
//                   console.log("asrResult: ",asrResult);

//               }

//               window.robot.on(jibo.RobotEvents.ASR_RESULT, asrResultFunction);
//               runtime.startHats("jibo_hey_jibo");
//           });
//           //touch screen event
//           window.robot.on(jibo.RobotEvents.SCREEN_TOUCH,function(result) {
//               screenTouch = true;
//               console.log("screenTouch: ",result);
//               screen = result;
//               runtime.startHats("jibo_screentouch");
//           });
//           //head touch 
//            window.robot.on(jibo.RobotEvents.HEAD_TOUCH,function(sensors) {
//               headTouchCount += 1;
//               handOn = JSON.stringify(sensors).indexOf('true')>= 0;
//               if (!headTouchTimer) {
//                   headTouchTimer = setTimeout(() => {
//                       resetHeadTouch();
//                       runtime.startHats("jibo_head_touch");
//                   } ,1000);
//               } 
//           });
//           //motion events
//           window.robot.on(jibo.RobotEvents.MOTION_COUNT,function(num, vec) {
//               motionCount = num;
//               motionVector = vec;
//           })
//           window.robot.on(jibo.RobotEvents.NUMBER_PEOPLE,function(num, vec) {
//               peopleCount = num;
//               personVector = vec;
//           })
...
//   });
// }
/* <script src="./missions/secret/renv-client.js"></script>
<script src="./missions/secret/media-images.js"></script>
*/
// toJson(){
...
class Scratch3Jibo {
    ...
    missionCommander (wblocks) {
        ...
        if (stepIdx < mission.steps.length){ // if ((window.robot) && (stepIdx < mission.steps.length)){
            ...
            if (STATE == 0){
			    ...
			    this.jiboSay(step.init.text).then(() => {
    ...
    captureImage (args, util) {
        ...
        let url = 'http://' + ip + ':8082/image/' + fileName;
        if (connected == true) {
    ...
    showPhoto (args, util) {
        let fileName = args.fileName;
        let url = 'http://' + ip + ':8082/./src/playground/assets/images/' + fileName;
        console.log(url);
        if (connected == true) {
